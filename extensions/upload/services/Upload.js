...
module.exports = {
  ...
  async upload(files, config, params={}) {
    ...
    const uploadFile = async file => {
      ...
      const res = await this.add(file, params);
  ...
  add(values, params) {
    values.type = typeMap[values.ext]
    values.tenantId = params.tenantId
  ...
  async remove(file, config, params={}) {
    // get upload provider settings to configure the provider to use
    ...
    return strapi.query('file', 'upload').delete({ 
      id: file.id, 
      tenantId: params.tenantId,
      appid: params.appid,
    });
