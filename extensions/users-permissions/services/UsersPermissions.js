...
module.exports = {
  ...
  async getRole(roleID, plugins, params) {
    const role = await strapi
      .query('role', 'users-permissions')
      .findOne({ id: roleID, ...params }, ['users', 'permissions']);
  ...
  async updateRole(roleID, body) {
    const [role, authenticated] = await Promise.all([
      ...
      strapi
        .query('role', 'users-permissions')
        .findOne({ type: 'authenticated', appid: body.appid, tenantId: body.tenantId }, []),
    ]);

    await strapi
      .query('role', 'users-permissions')
      .update({ id: roleID, appid: body.appid, tenantId: body.tenantId }, _.pick(body, ['name', 'description']));
