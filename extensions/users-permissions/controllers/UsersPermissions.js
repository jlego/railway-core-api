...
module.exports = {
  ...
  async getRole(ctx) {
    const { id } = ctx.params;
    const { lang, appid, tenantId } = ctx.query;
    ...
    const role = await strapi.plugins[
      'users-permissions'
    ].services.userspermissions.getRole(id, plugins, { appid, tenantId });
  ...
  async getRoles(ctx) {
    const { appid, tenantId } = ctx.query;
    try {
      const roles = await strapi.plugins[
        'users-permissions'
      ].services.userspermissions.getRoles({ appid, tenantId });
